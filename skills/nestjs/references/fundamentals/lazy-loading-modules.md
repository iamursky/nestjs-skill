...
> warning **Warning** [Lifecycle hook methods](https://docs.nestjs.com/fundamentals/lifecycle-events) are not invoked in lazy loaded modules and services.
...
The `LazyModuleLoader#load` method returns the [module reference](https://docs.nestjs.com/fundamentals/module-ref) (of `LazyModule`), which lets you navigate the internal list of providers and obtain a reference to any provider, using its injection token as a lookup key.
...
> error **Warning** Controllers, [resolvers](https://docs.nestjs.com/graphql/resolvers), and [gateways](https://docs.nestjs.com/websockets/gateways) registered inside lazy loaded modules will not behave as expected. Similarly, you cannot register middleware (by implementing the `NestModule` interface's `configure()` method with the `MiddlewareConsumer`) on demand.
