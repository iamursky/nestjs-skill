...
3. In `app.module.ts`, we associate the token `CatsService` with the class `CatsService` from the `cats.service.ts` file. The <a href="https://docs.nestjs.com/fundamentals/custom-providers#standard-providers">Standard providers</a> section below shows exactly how this association (also called _registration_) occurs.
...
The standard [constructor-based injection](https://docs.nestjs.com/providers#dependency-injection) pattern **requires** the dependency to be declared with a class name. The `'CONNECTION'` custom provider, however, uses a string-valued token. To inject such a provider, use the `@Inject()` decorator, which takes a single argument: the token.
