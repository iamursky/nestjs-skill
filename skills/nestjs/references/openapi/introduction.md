...
4. An optional configuration object (see [setup options](https://docs.nestjs.com/openapi/introduction#setup-options))
...
> info **Hint** Swagger UI works with the default [Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) of Nest's built-in [security headers](https://docs.nestjs.com/security/helmet). If you add scripts through `customJs`/`customJsStr`, or serve Swagger UI over plain HTTP outside local development, see [Swagger UI and GraphQL IDEs](https://docs.nestjs.com/security/helmet#swagger-ui-and-graphql-ides) for the directives to adjust.
...
Nest route parameter decorators accept a [Standard Schema](https://standardschema.dev/) compatible schema through their `schema` option (see the [controllers](https://docs.nestjs.com/controllers#request-object) chapter):
