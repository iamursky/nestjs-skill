...
With the [Swagger plugin enabled](https://docs.nestjs.com/openapi/cli-plugin#using-the-cli-plugin), you can declare the class above without any decorators:
...
> info **Hint** When you use [mapped type utilities](https://docs.nestjs.com/openapi/mapped-types) (like `PartialType`) in DTOs, import them from `@nestjs/swagger` instead of `@nestjs/mapped-types` so that the plugin picks up the schema.
...
For standard (non-monorepo) setups, using CLI plugins with the SWC builder requires type checking to be enabled, as described in the [SWC recipe](https://docs.nestjs.com/recipes/swc#type-checking).

```bash
$ nest start -b swc --type-check
```

For monorepo setups, follow the [monorepo and CLI plugins](https://docs.nestjs.com/recipes/swc#monorepo-and-cli-plugins) instructions to create a `generate-metadata.ts` file, and then run it:
