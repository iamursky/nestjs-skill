...
Starting with NestJS v12, gateways support [request-scoped](https://docs.nestjs.com/fundamentals/injection-scopes) providers. Nest creates a new instance of every request-scoped dependency per connected socket. That instance lives as long as the connection does, so it can safely hold per-connection state.
