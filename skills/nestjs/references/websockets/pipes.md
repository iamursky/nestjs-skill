...
There is no fundamental difference between [regular pipes](https://docs.nestjs.com/pipes) and WebSocket pipes. The only difference is that instead of throwing `HttpException`, you should throw `WsException`. Only a `WsException` reaches the client with its own message. Any other exception, including the `BadRequestException` that the built-in validation pipes throw by default, is reported to the client as `'Internal server error'`.
