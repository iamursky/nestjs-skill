...
A request that would be rejected is still allowed when it matches an [excluded route](https://docs.nestjs.com/security/csrf#excluding-routes), or when its `Origin` is one of the [trusted origins](https://docs.nestjs.com/security/csrf#trusted-origins).
...
- `app.useSecurityHeaders()` (see [Security headers](https://docs.nestjs.com/security/helmet)) shares the same request hook. You can call the two methods in any order: the headers are always written before the CSRF check runs, so rejected requests carry them too.
...
A rejected request becomes a `ForbiddenException`, which goes through your global [exception filters](https://docs.nestjs.com/exception-filters) like any other exception. Without a custom filter, the client receives a `403` response like this:
...
> warning **Warning** Origins allowed by [CORS](https://docs.nestjs.com/security/cors) are not trusted automatically. If a cross-origin frontend sends state-changing requests with credentials, list its origin in `trustedOrigins` too.
...
Exclusions are declared like the ones of `MiddlewareConsumer.exclude()` (see [Middleware](https://docs.nestjs.com/middleware)): as a path, which matches every request method, or as a `RouteInfo` object with a `path`, a `method`, and an optional `version`. Paths are declared without the global prefix:
...
- The [global prefix](https://docs.nestjs.com/faq/global-prefix) is added, unless the route is excluded from it.
- With [URI versioning](https://docs.nestjs.com/http/versioning), the version segment of the `version` property is added. The default version is not applied, so set `version` on the exclusion of a versioned route.
...
- The `Origin`/`Host` fallback does not know the scheme. For browsers that don't send `Sec-Fetch-Site`, a request from `http://example.com` to `https://example.com` is allowed (fails open), as in Go. [HSTS](https://docs.nestjs.com/security/helmet#default-headers), which `app.useSecurityHeaders()` sends by default, mitigates this.
...
- WebSocket upgrade requests are not checked. If your [gateways](https://docs.nestjs.com/websockets/gateways) authenticate connections with cookies, validate the handshake's `Origin` header yourself.
