...
> info **Hint** In a real application, use a DTO class instead of the `Record<string, any>` type to define the shape of the request body. See the [validation](https://docs.nestjs.com/application/validation) chapter for more information.
...
If most of your endpoints should be protected by default, you can register the authentication guard as a [global guard](https://docs.nestjs.com/guards#binding-guards). Instead of using the `@UseGuards()` decorator on each controller, you then flag which routes are public.
...
Finally, the `AuthGuard` must return `true` when it finds the `"isPublic"` metadata. To read the metadata, use the `Reflector` class (see [Putting it all together](https://docs.nestjs.com/guards#putting-it-all-together) in the guards chapter).
...
To learn how, see the [Passport recipe](https://docs.nestjs.com/recipes/passport).
