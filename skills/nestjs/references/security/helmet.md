...
The headers are set on every response: route responses, `404` responses, error responses, responses rejected by the [CSRF protection](https://docs.nestjs.com/security/csrf), and Server-Sent Events streams. The `X-Powered-By` header is removed. The following rules apply:

- Call `useSecurityHeaders()` before `app.init()` or `app.listen()`. Calling it afterwards, or calling it twice, throws an error.
- Options are validated when you call the method, so a bad configuration fails at startup (see [Configuring headers](https://docs.nestjs.com/security/helmet#configuring-headers)).
...
| ----------------------------------- | --------------------------------------------------------------- | ------------------------------- |
| `Content-Security-Policy`           | See [Content Security Policy](https://docs.nestjs.com/security/helmet#content-security-policy) | `contentSecurityPolicy`         |
...
| Option                          | Properties                                                                                                                                            |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `contentSecurityPolicy`         | `directives`, `useDefaults` (default `true`) and `reportOnly` (default `false`). See [Content Security Policy](https://docs.nestjs.com/security/helmet#content-security-policy). |
...
A route can still override a header with the [`@Header()`](https://docs.nestjs.com/controllers#response-headers) decorator, for example to let other sites load an image:
...
[Apollo Sandbox](https://docs.nestjs.com/graphql/quick-start#apollo-sandbox), enabled with `ApolloServerPluginLandingPageLocalDefault()`, loads its scripts, images and manifest from Apollo's CDN, and embeds the Sandbox in a frame:
