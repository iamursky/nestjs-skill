...
To enable the plugin, open `nest-cli.json` (if you use the [Nest CLI](https://docs.nestjs.com/cli/overview)) and add the following `plugins` configuration:
...
For standard (non-monorepo) setups, using CLI plugins with the SWC builder requires type checking to be enabled, as described in [SWC type checking](https://docs.nestjs.com/recipes/swc#type-checking).

```bash
$ nest start -b swc --type-check
```

For monorepo setups, follow the instructions in [monorepo and CLI plugins](https://docs.nestjs.com/recipes/swc#monorepo-and-cli-plugins).
