...
You can also build your own driver (see [Creating a custom driver](https://docs.nestjs.com/graphql/other-features#creating-a-custom-driver)).
...
> info **Hint** The following chapters use the `@nestjs/apollo` package. To use Mercurius instead, see [Mercurius integration](https://docs.nestjs.com/graphql/quick-start#mercurius-integration).
...
> info **Hint** Guards, interceptors, and other enhancers running in the GraphQL context can reach the same object via `GqlExecutionContext.create(context).getContext()` (see [Other features](https://docs.nestjs.com/graphql/other-features)).
...
If your application uses [subscriptions](https://docs.nestjs.com/graphql/subscriptions), use `graphql-ws`. Support for `subscriptions-transport-ws` has been **removed**: it is no longer accepted as a `subscriptions` key, and the package is no longer a dependency.
...
Instead of Apollo, [Fastify](https://docs.nestjs.com/http/performance) users can use the `@nestjs/mercurius` driver.
