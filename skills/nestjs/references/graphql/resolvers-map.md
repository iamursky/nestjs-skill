...
> info **Hint** In addition to GraphQL's built-in scalar types, you can define [custom scalar types](https://docs.nestjs.com/graphql/scalars).
...
You can define multiple resolver classes, and Nest combines them at run time. See the [Module](https://docs.nestjs.com/graphql/resolvers#module) section below for more on code organization.
...
> info **Hint** The Nest CLI provides a generator (schematic) that generates **all the boilerplate code** for you. See the [CRUD generator](https://docs.nestjs.com/recipes/crud-generator) recipe for details.
...
Use the `@Args()` decorator to extract arguments from a request for use in the handler method. This works much like [REST route parameter extraction](https://docs.nestjs.com/controllers#route-parameters).
...
> info **Hint** Again, due to the limitations of TypeScript's metadata reflection system, you must either use the `@Field()` decorator to indicate type and optionality, or use the [CLI plugin](https://docs.nestjs.com/graphql/cli-plugin). As before, you don't need to add `null` or `undefined` to the type of the nullable `firstName` field, but you do need to guard against these values in your resolvers.
...
> info **Hint** Argument classes like `GetAuthorArgs` work well with the `ValidationPipe` (see [Validation](https://docs.nestjs.com/application/validation)).
...
As mentioned in the [Quick start](https://docs.nestjs.com/graphql/quick-start) chapter, in the schema first approach you start by manually defining schema types in SDL (see the [GraphQL type language](https://graphql.org/learn/schema/#type-language)). Consider the following SDL type definitions.
...
If you use the schema first approach and have enabled typings generation (with `outputAs: 'class'`, as shown in the [Quick start](https://docs.nestjs.com/graphql/quick-start) chapter), running the application generates the following file in the location you specified in the `GraphQLModule.forRoot()` method, e.g., `src/graphql.ts`:
...
> warning **Notice** To enable automatic validation of your inputs (and parameters), use `ValidationPipe`. Learn more in the [Validation](https://docs.nestjs.com/application/validation) and [Pipes](https://docs.nestjs.com/pipes) chapters.
