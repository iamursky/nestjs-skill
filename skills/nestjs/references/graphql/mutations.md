...
Add another method to the `AuthorsResolver` from the [Resolvers](https://docs.nestjs.com/graphql/resolvers) chapter.
...
The `upvotePost()` method takes `postId` (`Int`) as an argument and returns the updated `Post` entity. For the reasons explained in the [Resolvers](https://docs.nestjs.com/graphql/resolvers) chapter, you have to set the expected type explicitly.
...
> info **Hint** The `@InputType()` decorator takes an options object as an argument, so you can, for example, specify the input type's description. Due to the limitations of TypeScript's metadata reflection system, you must either use the `@Field()` decorator to indicate a type, or use the [CLI plugin](https://docs.nestjs.com/graphql/cli-plugin).
...
Extend the `AuthorsResolver` from the [Resolvers](https://docs.nestjs.com/graphql/resolvers) chapter.
