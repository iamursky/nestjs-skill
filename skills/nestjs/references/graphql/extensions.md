...
The logic that uses the custom metadata can be as complex as needed. For example, you can create an interceptor that stores or logs events per method invocation, or a [field middleware](https://docs.nestjs.com/graphql/field-middleware) that matches the roles required to retrieve a field against the caller's permissions (a field-level permissions system).
