...
> info **Hint** When you use this approach to perform dependency injection for the filter, the filter is global regardless of the module in which this construction is employed. Register it in the module where the filter (`HttpExceptionFilter` in the example above) is defined. Also, `useClass` is not the only way to register a custom provider. Learn more about [custom providers](https://docs.nestjs.com/fundamentals/custom-providers).

> info **Hint** Exceptions thrown from [middleware](https://docs.nestjs.com/middleware#error-handling) are also processed by the exceptions layer. Because middleware runs before a route handler is selected, only **global** exception filters apply (`app.useGlobalFilters()` or `APP_FILTER`). Method-scoped and controller-scoped `@UseFilters()` bindings are not invoked.
...
The example below is platform-agnostic: it delivers the response through the [HTTP adapter](https://docs.nestjs.com/faq/http-adapter) rather than using the platform-specific `Request` and `Response` objects directly, so the same filter works with both Express and Fastify.
...
The second is to register the filter with the `APP_FILTER` token, <a href="https://docs.nestjs.com/exception-filters#binding-filters">as shown earlier</a>.
