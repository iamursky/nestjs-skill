...
This article walks you through migrating from NestJS version 11 to version 12. Version 12 centers on ESM packages, updated CLI defaults, first-class support for [Standard Schema](https://standardschema.dev/)-based validation and serialization, and native [observability](https://docs.nestjs.com/observability/overview) support.
...
This is a schema-first alternative to the traditional `ValidationPipe` plus `class-validator` flow. The same schemas can also feed OpenAPI generation (see [Standard Schema (Zod, Valibot)](https://docs.nestjs.com/openapi/introduction#standard-schema-zod-valibot)).
...
Because validation is no longer tied to a single library, we now recommend a modern Standard Schema library such as Zod for new projects, and the [Configuration chapter](https://docs.nestjs.com/application/configuration#schema-validation) has been rewritten around it.
...
The `timeout` option of the built-in database, microservice, and gRPC indicators (e.g., `db.pingCheck('database', { timeout: 1500 })`) is deprecated. Chain `.withTimeout(1500)` on the returned attempt instead. See the [Terminus chapter](https://docs.nestjs.com/recipes/terminus#timeouts-and-caching) for details.
...
Both default to the previous behavior, so existing applications are unaffected unless you set them. See the [Controllers chapter](https://docs.nestjs.com/controllers#route-conflicts-and-resolution-order) for the full description.
...
See [Machine-readable error codes](https://docs.nestjs.com/exception-filters#machine-readable-error-codes) in the Exception filters chapter.
...
In JSON mode, they are nested under a `params` key, or spread into the root object if you enable `flattenParams`. This behavior is on by default in v12; set `structuredParams: false` to restore the previous behavior. See [Structured logging params](https://docs.nestjs.com/application/logger#structured-logging-params) in the Logger chapter.

## Native observability support

Version 12 adds first-class observability support through the official [`@nestjs/observe`](https://docs.nestjs.com/observability/overview) SDK. Instead of attaching a generic Node.js APM agent to the HTTP server, the SDK plugs into Nest's own request lifecycle through the `instrument` application option. As a result, requests, jobs, errors, and traces are reported in terms of your controllers, providers, resolvers, and queue consumers.
...
There is nothing to migrate here: it is a new, opt-in capability. See the [Observability chapter](https://docs.nestjs.com/observability/overview) for what auto-instrumentation covers, and the [SDK reference](https://docs.nestjs.com/observability/sdk) for configuration options.
...
- **gRPC exception filter**: `GrpcExceptionFilter`, plus a family of status-specific exceptions, maps errors to the proper gRPC status codes instead of `UNKNOWN`. See the [gRPC chapter](https://docs.nestjs.com/microservices/grpc#exception-handling).
- **Regex Kafka patterns**: `@MessagePattern()` and `@EventPattern()` now accept a `RegExp` on the Kafka transport. See the [Kafka chapter](https://docs.nestjs.com/microservices/kafka#regular-expression-patterns).
- **Request-scoped WebSocket gateways**: gateways now support request-scoped providers, with the socket injectable through the `REQUEST` token. See the [Gateways chapter](https://docs.nestjs.com/websockets/gateways#request-scoped-gateways).
