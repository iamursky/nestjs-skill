...
In both cases, pipes operate on the arguments being processed by a <a href="https://docs.nestjs.com/controllers#route-parameters">controller route handler</a>. Nest interposes a pipe immediately before a method is invoked; the pipe receives the arguments destined for the method and operates on them. Any transformation or validation takes place at that point, after which the route handler is invoked with the (potentially) transformed arguments.
...
> info **Hint** See [Validation techniques](https://docs.nestjs.com/application/validation) for extensive examples of validation pipes.
...
<table>
  <tr>
    ...
    <td>Indicates whether the argument is a body
      ...
      <a href="https://docs.nestjs.com/custom-decorators">custom decorators</a>).</td>
  </tr>
  <tr>
    <td>
      <code>metatype</code>
...
> info **Hint** The following sections build validation pipes from scratch to show how pipes work. For schema-based validation in production, use the built-in `StandardSchemaValidationPipe` instead. It works with Zod, Valibot, ArkType, and any other [Standard Schema](https://standardschema.dev/) compatible library, and reads the schema you attach through the `schema` option of parameter decorators such as `@Body()` and `@Param()`. See [Schema-based validation](https://docs.nestjs.com/application/validation#schema-based-validation) for details.
...
> warning **Notice** In a hybrid application, `useGlobalPipes()` doesn't set up pipes for connected microservices by default (see [Hybrid application](https://docs.nestjs.com/faq/hybrid-application) to change this behavior). In a standard (non-hybrid) microservice application, `useGlobalPipes()` does mount the pipes globally.
