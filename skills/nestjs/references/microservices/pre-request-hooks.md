...
Exceptions thrown inside a hook are caught by the same `RpcProxy` wrapper that covers the entire pipeline, so existing [exception filters](https://docs.nestjs.com/microservices/exception-filters) handle them without extra configuration.
