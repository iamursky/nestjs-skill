> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/microservices/pre-request-hooks.md
...
Exceptions thrown inside a hook are caught by the same wrapper that covers the rest of the pipeline, so your existing [exception filters](https://docs.nestjs.com/microservices/exception-filters) handle them without extra configuration.
...
> info **Note** In a [hybrid application](https://docs.nestjs.com/faq/hybrid-application), `connectMicroservice()` initializes the connected microservice immediately (unless you pass `deferInitialization: true` in its second argument), so hooks registered on the returned instance afterward are not applied.
