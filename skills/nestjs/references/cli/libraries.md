...
The same mechanism lets you build and deploy applications that compose libraries. Once you've imported `MyLibraryModule`, `nest build` handles module resolution automatically and bundles the application, along with any library dependencies, for deployment. The default compiler for a monorepo is **Rspack**, so the output is a single file that bundles all the transpiled JavaScript. You can switch to `tsc` as described in [Global compiler options](https://docs.nestjs.com/cli/monorepo#global-compiler-options).
