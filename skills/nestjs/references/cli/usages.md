...
Creates and initializes a new Nest project. The command prompts for any information you don't pass as an option: the project name, the package manager, the module system (ESM or CommonJS), and whether to set up [NestJS Observe](https://docs.nestjs.com/observability/overview).
...
| `resource`    | `res` | Generate a new CRUD resource (TypeScript only). See the [CRUD (resource) generator](https://docs.nestjs.com/recipes/crud-generator).          |
...
| `--watchAssets`         | Watch non-TS files (assets such as `.graphql` files). See [Assets](https://docs.nestjs.com/cli/monorepo#assets) for more details.                                                                                 |
...
- Optionally installs and wires up [`@nestjs/observe`](https://docs.nestjs.com/observability/overview). It prompts, unless you pass `--observe` or `--no-observe`
...
> info **Hint** The command deliberately does not migrate your project to ESM, Vitest, or oxlint. Those are the defaults for newly generated v12 projects, but existing projects can adopt them on their own schedule. See the [migration guide](https://docs.nestjs.com/migration-guide) for the full picture.
...
Once Mau starts, it owns the terminal, so its output and prompts reach you directly. See the [Deployment chapter](https://docs.nestjs.com/deployment#easy-deployment-with-mau) for what Mau does and how to configure it.
