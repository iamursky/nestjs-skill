...
For an agent that needs to keep asking - follow a trace into its logs, check whether an error is still firing - connect it to the [MCP server](https://docs.nestjs.com/observability/mcp-server) instead.
...
The same walk is available to an agent over the [MCP server](https://docs.nestjs.com/observability/mcp-server), one tool per step, when you'd rather ask questions than click through them.
