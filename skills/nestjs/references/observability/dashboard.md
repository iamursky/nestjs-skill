> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/observability/dashboard.md
...
![](https://www.observe.nestjs.com/docs/telemetry/dashboard.webp)
...
| **Traces**   | The waterfall for a trace id, across every service that shares it - see [Distributed tracing](https://docs.nestjs.com/observability/distributed-tracing).                                                                                                                                                                              |
...
| **Custom**   | Counters, gauges, and summaries reported through [`TracerService`](https://docs.nestjs.com/observability/manual-instrumentation#custom-metrics), grouped by metric name and labels.                                                                                                                                                             |
...
![](https://www.observe.nestjs.com/docs/telemetry/services.webp)
...
When the agent kept a request's own inputs (because it failed, or ran past the [`slowerThanMs`](https://docs.nestjs.com/observability/sdk#capturing-failed-and-slow-requests) threshold you configured), the request's **Details** card has two extra folded rows, *Request headers* and *Request body*. A request with nothing captured has no such rows.
...
![](https://www.observe.nestjs.com/docs/telemetry/copy-agent-prompt.webp)

For an agent that needs to keep asking questions (follow a trace into its logs, check whether an error is still firing), connect it to the [MCP server](https://docs.nestjs.com/observability/mcp-server) instead.
...
![](https://www.observe.nestjs.com/docs/alerts/create-alert.webp)
...
The same path is available to an agent over the [MCP server](https://docs.nestjs.com/observability/mcp-server), one tool per step, when you'd rather ask questions than click through views.
