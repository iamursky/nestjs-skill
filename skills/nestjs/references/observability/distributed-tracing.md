...
If you use the [HTTP module](https://docs.nestjs.com/techniques/http-module), the same header can be added in an Axios request interceptor once, rather than at every call site:
...
See the [gRPC](https://docs.nestjs.com/microservices/grpc) chapter for how metadata is read and written on both sides of a call.
...
> warning **Warning** A service that only receives RPC calls still has a `traceIdGenerator` that expects an HTTP request by default. In a [hybrid application](https://docs.nestjs.com/faq/hybrid-application) that serves both HTTP and a microservice transport, write the generator defensively - check for `headers` first, then fall back to the payload.
...
- **Failing spans are marked**, and the execution page leads with an error card for the first span that threw - class, message, the trimmed stack trace with the throwing frame marked, and the source lines around it when [`sourceContext`](https://docs.nestjs.com/observability/sdk#error-source-context) is on.
...
![](https://www.observe.nestjs.com/docs/telemetry/traces.webp)

From any failed or unusually slow execution, the **Copy agent prompt** button packages the whole page - context, error, stack trace with source, top spans by self time, and the logs - as a self-contained markdown prompt for a coding agent. See [Dashboard](https://docs.nestjs.com/observability/dashboard#handing-a-failure-to-a-coding-agent).
