> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/observability/sdk.md
...
| ---------------- | -------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `appKey`         | `string` | required | Generated from your project's **API Keys** page - see [First project](https://docs.nestjs.com/observability/overview#first-project). Keep it out of source control.                                                              |
...
![](https://www.observe.nestjs.com/docs/sdk/deployments.webp)
...
![](https://www.observe.nestjs.com/docs/telemetry/sql.webp)
...
`@nestjs/bullmq` and the original `@nestjs/bull` are both instrumented: every processor run is reported as a job, with its queue wait, attempt number, and outcome. A job added from inside a traced operation inherits that operation's trace id - see [Distributed tracing](https://docs.nestjs.com/observability/distributed-tracing#queue-jobs-bullmq-and-bull).
...
Everything captured passes through [`redaction`](https://docs.nestjs.com/observability/sdk#logs) before it leaves the process: sensitive keys (`password`, `token`, `authorization`, ...) are masked by name inside the body, and naming a sensitive header records `[REDACTED]` instead of its value. `capture: false` records nothing. The capture is shown on the request's detail page as a **Request** card.
...
Because the default generator adopts an incoming `x-request-id`, a chain of HTTP services already shares one trace with no extra configuration. The same applies to the TCP and Redis microservice transports on `@nestjs/microservices` 12.0.4 or later. Propagating across gRPC, the other microservice transports, and GraphQL takes a few lines - see [Distributed tracing](https://docs.nestjs.com/observability/distributed-tracing).
...
- [Manual instrumentation](https://docs.nestjs.com/observability/manual-instrumentation) - add your own spans, capture handled errors, attach request-scoped attributes, and report custom metrics.
- [Distributed tracing](https://docs.nestjs.com/observability/distributed-tracing) - keep one trace across HTTP, gRPC, microservice transports, and GraphQL.
- [Dashboard](https://docs.nestjs.com/observability/dashboard) - what shows up once the SDK is running, and how the views relate to each other.
