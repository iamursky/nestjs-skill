> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/observability/overview.md
...
- **Distributed tracing** with a waterfall of every span a request ran, self-time ranking to find where the time actually went, and correlation across services once a trace id is propagated (see [Distributed tracing](https://docs.nestjs.com/observability/distributed-tracing)).
...
Paid plans add **logs streaming** (correlated to traces, with redaction on by default), **alerting** (thresholds, anomaly detection, absence detection, Slack/webhook/email channels), **issues**, team management, and - on the higher tiers - **SSO**, **SLOs** with error budgets and burn-rate alerts, and an **MCP server** so an MCP-compatible agent can query your telemetry directly and set up a new service (see [MCP server](https://docs.nestjs.com/observability/mcp-server)).
...
![](https://www.observe.nestjs.com/docs/overview/projects.webp)
...
4. **Instrument your application** by following the [SDK](https://docs.nestjs.com/observability/sdk) chapter. Telemetry starts appearing within moments of your application receiving traffic.

> info **Hint** If you're setting up with a coding agent, use the **Copy setup prompt** button in the dialog that shows a new key's secret. It combines the install command, the three code edits, and the credentials into one prompt. An agent connected to the [MCP server](https://docs.nestjs.com/observability/mcp-server#setting-up-a-service) can create the application and the key itself.

![](https://www.observe.nestjs.com/docs/applications/api-keys.webp)
...
- [SDK](https://docs.nestjs.com/observability/sdk) - installation, the three-line quick start, and every configuration option.
- [Manual instrumentation](https://docs.nestjs.com/observability/manual-instrumentation) - custom spans, handled errors, request-scoped attributes, and custom metrics.
- [Distributed tracing](https://docs.nestjs.com/observability/distributed-tracing) - propagating a trace id across HTTP, gRPC, microservice transports, and GraphQL.
- [Error monitoring](https://docs.nestjs.com/observability/error-monitoring) - what gets captured, source context, grouping errors into defects, and getting from an error to a verified fix.
- [Dashboard](https://docs.nestjs.com/observability/dashboard) - what each view shows once telemetry is flowing, and how alerts, SLOs, and issues build on it.
- [MCP server](https://docs.nestjs.com/observability/mcp-server) - querying your telemetry from Claude Code, Cursor, VS Code, or your own agent.
