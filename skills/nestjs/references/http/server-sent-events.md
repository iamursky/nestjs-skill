> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/http/server-sent-events.md
...
> warning **Warning** Server-Sent Events routes must return an `Observable` stream (or a `Promise` that resolves to one, as shown [below](https://docs.nestjs.com/http/server-sent-events#client-disconnection)).
