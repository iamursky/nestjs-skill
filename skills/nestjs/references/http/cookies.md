> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/http/cookies.md
...
As with `@Query()` or `@Param()`, you can pass [pipes](https://docs.nestjs.com/pipes) after the cookie name:
...
Both methods take the native response object as their first argument. In a route handler, inject the `HttpAdapterHost` (see [HTTP adapter](https://docs.nestjs.com/faq/http-adapter)) and get hold of the response object with the `@Res()` decorator, with its `passthrough` option enabled:
...
> warning **Warning** To leave the response handling logic to the framework, set the `passthrough` option to `true`, as shown above. See the [library-specific approach](https://docs.nestjs.com/controllers#library-specific-approach) section of the Controllers chapter for details.
...
> warning **Warning** A `secret` that is `undefined`, for example because an environment variable is not set, counts as no secret at all. The application starts, and the error only surfaces when a request sets or reads a signed cookie. Validate your configuration at startup, for example as described in the [Configuration](https://docs.nestjs.com/application/configuration#schema-validation) chapter.
