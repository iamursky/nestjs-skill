> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/http/versioning.md
...
> warning **Notice** With URI Versioning, the version is automatically added to the URI after the <a href="https://docs.nestjs.com/faq/global-prefix">global path prefix</a> (if one exists) and before any controller or route paths.
...
[Middleware](https://docs.nestjs.com/middleware) can also use versioning metadata to target a specific route version. To do so, set the `version` property in the route object passed to the `MiddlewareConsumer.forRoutes()` method:
