> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/application/serialization.md
...
> info **Hint** Neither interceptor serializes [StreamableFile](https://docs.nestjs.com/http/file-upload#streaming-files) responses.
...
The interceptor can also be bound application-wide (see [Binding interceptors](https://docs.nestjs.com/interceptors#binding-interceptors)). Together, the interceptor and the entity class declaration ensure that **every** route handler that returns a `UserEntity` omits the `password` property, which enforces this rule in one place.
...
```typescript title="user-response.schema.ts"
import { z } from 'zod';
...
Alternatively, register it with the `APP_INTERCEPTOR` token, as described in [Binding interceptors](https://docs.nestjs.com/interceptors#binding-interceptors), and Nest injects the `Reflector` for you. Handlers without a schema are not affected by the global interceptor.
