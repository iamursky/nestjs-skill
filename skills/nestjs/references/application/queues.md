> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/application/queues.md
...
- `extraOptions` - Extra options for module initialization. See [Manual registration](https://docs.nestjs.com/application/queues#manual-registration).
...
Each queue can have one or many producers, consumers, and listeners. Consumers retrieve jobs from the queue in a specific order: FIFO (the default), LIFO, or according to priorities. The <a href="https://docs.nestjs.com/application/queues#job-options">job options</a> section explains how to control the processing order.
...
Job producers add jobs to queues. Producers are typically application services (Nest [providers](https://docs.nestjs.com/providers)). To add jobs to a queue, first inject the queue into the service:
...
When a consumer is flagged as request-scoped (see [injection scopes](https://docs.nestjs.com/fundamentals/injection-scopes#provider-scope)), a new instance of the class is created exclusively for each job. The instance is garbage-collected after the job completes.
...
Worker events must be declared within a <a href="https://docs.nestjs.com/application/queues#consumers">consumer</a> class (i.e., within a class decorated with the `@Processor()` decorator). To listen for an event, use the `@OnWorkerEvent(event)` decorator with the event you want to handle. For example, to listen to the event emitted when a job enters the active state in the `audio` queue, use the following construct:
...
The factory behaves like any other [asynchronous provider](https://docs.nestjs.com/fundamentals/async-providers) (e.g., it can be `async` and can inject dependencies through `inject`).
...
Jobs can have names. This lets you create specialized <a href="https://docs.nestjs.com/application/queues#consumers">consumers</a> that only process jobs with a given name.
...
> warning **Warning** When using named jobs, you must create a processor for each unique name added to a queue, or the queue reports that a processor for the given job is missing. See <a href="https://docs.nestjs.com/application/queues#consumers">consumers</a> for more information on consuming named jobs.
...
Event listeners must be declared within a <a href="https://docs.nestjs.com/application/queues#consumers">consumer</a> class (i.e., within a class decorated with the `@Processor()` decorator). To listen for an event, use one of the decorators in the table below to declare a handler for the event. For example, to listen to the event emitted when a job enters the active state in the `audio` queue, use the following construct:
