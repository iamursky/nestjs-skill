> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/application/http-client.md
...
The package has no dependencies besides `@nestjs/common` and `@nestjs/core`. Every method returns a Promise, so you `await` requests and handle failures with `try`/`catch`. If you prefer Observables, see [Working with RxJS](https://docs.nestjs.com/application/http-client#working-with-rxjs).

> info **Hint** This package replaces the Axios-based `HttpModule` from `@nestjs/axios`, which this chapter used to describe. `@nestjs/axios` remains available. See [Migrating from @nestjs/axios](https://docs.nestjs.com/application/http-client#migrating-from-nestjsaxios) to switch.
...
```typescript title="repos.module.ts"
@Module({
  imports: [
    HttpClientModule.register({
      name: 'github',
...
```typescript title="repos.service.ts"
@Injectable()
...
To build a client's options from other providers, such as the `ConfigService` from the [configuration](https://docs.nestjs.com/application/configuration#getting-started) chapter, use `registerAsync()`:
...
```typescript title="github-client.config.ts"
@Injectable()
...
- `context` is a free-form object that [interceptors](https://docs.nestjs.com/application/http-client#interceptors) can read.
...
| `'stream'` | A Node.js `Readable`. See [Streaming responses](https://docs.nestjs.com/application/http-client#streaming-responses). |
...
Nest's built-in exception filter [doesn't log](https://docs.nestjs.com/exception-filters#exceptions-logging) an `HttpException`, so a `502` or `504` would leave no trace. `toHttpException()` therefore logs every failure that it turns into a `5xx`, with the `HttpClient` context:
...
Instead of mapping errors in every service, you can let them leave the handler unchanged and map them in an [exception filter](https://docs.nestjs.com/exception-filters#binding-filters). This keeps call sites free of `try`/`catch`, and interceptors further out still see the original error:

```typescript title="http-client-error.filter.ts"
...
```typescript title="api-key.interceptor.ts"
...
```typescript title="github-auth.interceptor.ts"
import { Injectable } from '@nestjs/common';
...
```typescript title="http-logging.interceptor.ts"
...
> info **Hint** For tracing, you don't need an interceptor. [NestJS Observe](https://docs.nestjs.com/observability/distributed-tracing#http-to-http) records every outbound `fetch` call as a span, and propagates the trace id to other Nest services.

## Streaming responses

To pass a large response through without buffering it in memory, set `responseType: 'stream'`. The `data` is a Node.js `Readable`, which you can return from a route handler in a [`StreamableFile`](https://docs.nestjs.com/http/file-upload#streaming-files):

```typescript title="invoices.controller.ts"
@Get(':id/pdf')
...
```typescript title="partner.module.ts"
...
- **Errors are safe to log**, as described in [Handling errors](https://docs.nestjs.com/application/http-client#handling-errors).
...
```typescript title="repos.service.spec.ts"
