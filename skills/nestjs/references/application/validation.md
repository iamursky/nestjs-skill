> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/application/validation.md
...
The [Pipes](https://docs.nestjs.com/pipes) chapter explains how pipes work and how to bind them to parameters, route handlers, controllers, or the whole application. This chapter builds on it and covers real-world use of the two validation pipes.
...
The `schema` option is supported by `@Body()`, `@Query()`, and `@Param()`, as well as by `@MessageBody()` in [gateways](https://docs.nestjs.com/websockets/gateways) and `@Payload()` in [microservices](https://docs.nestjs.com/microservices/basics). Pass it as the only argument to validate the whole object, or after a property name to validate a single value:
...
```typescript title="list-users-query.dto.ts"
import { z } from 'zod';
...
```typescript title="update-user.dto.ts"
import { z } from 'zod';
...
Parameter decorators created with [`createParamDecorator()`](https://docs.nestjs.com/custom-decorators) accept the `schema` option too. For example, the following decorator extracts a tenant identifier from a request header:

```typescript title="tenant-id.decorator.ts"
...
> info **Hint** The same schemas can describe your API in the OpenAPI document. See [Standard Schema (Zod, Valibot)](https://docs.nestjs.com/openapi/introduction#standard-schema-zod-valibot) in the OpenAPI chapter.
