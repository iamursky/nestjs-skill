> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/application/file-storage.md
...
Start with the configuration, a [custom configuration file](https://docs.nestjs.com/application/configuration#custom-configuration-files) for `@nestjs/config`. `STORAGE_DRIVER` will pick local disks or S3 [in production](https://docs.nestjs.com/application/file-storage#use-s3-or-r2-in-production); until then, everything is local:
...
```typescript title="storage/storage.config.ts"
...
Photos may be JPEG, PNG or WebP images of up to 5 MiB. The rules are shared by the upload endpoint below and the [mobile uploads](https://docs.nestjs.com/application/file-storage#upload-straight-to-storage-from-the-mobile-app) further down:

```typescript title="products/photo-rules.ts"
...
```typescript title="products/photos.controller.ts"
...
```typescript title="products/product.ts"
...
```typescript title="products/photos.service.ts"
...
```typescript title="invoices/render-invoice.ts"
...
```typescript title="invoices/invoices.service.ts"
...
```typescript title="files/files.controller.ts"
...
```typescript title="test/photos.e2e-spec.ts"
...
| --- | --- | --- |
| `disks` | none (required) | Disks by name: instances, or `StorageDisk` classes that Nest instantiates, which can inject. See [Configure the disks](https://docs.nestjs.com/application/file-storage#configure-the-disks). |
...
| `signedUrl(key, options)` | A download link. Options: `expiresIn` (default 15 minutes, at most 7 days), `filename`, `disposition`. See [Keep invoices private](https://docs.nestjs.com/application/file-storage#keep-invoices-private). |
| `signedUpload(key, options)` | `url`, `method` (`PUT`), `headers` and `expiresAt` for a direct upload. Options: `contentType` (required), `contentLength`, `expiresIn`. See [Upload straight to storage from the mobile app](https://docs.nestjs.com/application/file-storage#upload-straight-to-storage-from-the-mobile-app). |
...
| `S3Disk` | `bucket` (required), `region`, `endpoint`, `forcePathStyle`, `credentials`, `prefix`, `serverSideEncryption`, `kmsKeyId`, `multipart`, `retry`, `timeout`, `batchDelete`, `fetch`, `publicUrl`. See [Use S3 or R2 in production](https://docs.nestjs.com/application/file-storage#use-s3-or-r2-in-production). |
| `InMemoryDisk` | `publicUrl`, `signedUrls`. For tests, with `keys()` and `clear()`. See [Testing](https://docs.nestjs.com/application/file-storage#testing). |
...
| --- | --- | --- |
| `uploadToDisk(options)` | `disk` (a name or an instance, default the default disk), `contentTypes`, `cacheControl`, `key`, `metadata` | A storage engine for `FileInterceptor()`. `key` and `metadata` are functions of the file and the request. See [Upload product photos](https://docs.nestjs.com/application/file-storage#upload-product-photos). |
...
| `serveSignedUrl(disk, options)` | `req`, `res`, `cacheControl` | Verifies and serves an app-served signed download URL. See [Serve signed URLs in development](https://docs.nestjs.com/application/file-storage#serve-signed-urls-in-development). |
