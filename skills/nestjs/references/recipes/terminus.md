...
> info **Hint** To create the module using the [Nest CLI](https://docs.nestjs.com/cli/overview), run the `$ nest g module health` command.
...
Health checks are executed by a [controller](https://docs.nestjs.com/controllers), which you can generate with the [Nest CLI](https://docs.nestjs.com/cli/overview).

```bash
$ nest g controller health
```

> info **Hint** We highly recommend enabling shutdown hooks in your application. When they are enabled, the Terminus integration uses this lifecycle event to report the `'shutting_down'` status and to apply the [graceful shutdown timeout](#graceful-shutdown-timeout). Learn more in the [application shutdown](https://docs.nestjs.com/fundamentals/lifecycle-events#application-shutdown) section.
...
[database](https://docs.nestjs.com/data/typeorm) chapter and make sure your application's database connection is established.
...
If your application uses [multiple databases](https://docs.nestjs.com/data/typeorm#multiple-databases), inject each
...
```typescript title="cache.health.ts"
@Injectable()
...
> info **Hint** To learn more about custom loggers in Nest, see [injecting a custom logger](https://docs.nestjs.com/application/logger#injecting-a-custom-logger).
...
|:------------------|:-----------------------------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------|
| `json`  (default) | Prints a summary of the health check result as a JSON object when a check fails                                                    | ![](https://docs.nestjs.com/assets/Terminus_Error_Log_Json.png)   |
| `pretty`          | Prints a summary of the health check result in formatted boxes when a check fails, highlighting successful and failed results      | ![](https://docs.nestjs.com/assets/Terminus_Error_Log_Pretty.png) |
