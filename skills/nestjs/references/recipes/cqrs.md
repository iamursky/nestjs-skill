...
>
> - Errors thrown in command handlers can still be caught by the built-in [exception filters](https://docs.nestjs.com/exception-filters).
> - Errors thrown in event handlers can't be caught by exception filters, so you have to handle them yourself: with a `try/catch` block, with a [saga](https://docs.nestjs.com/recipes/cqrs#sagas) that triggers a compensating event, or with another approach of your choice.
> - The value returned by a command handler can still be sent back to the client in the HTTP response.
> - Event handlers can't send HTTP responses. To send information to the client, use [WebSockets](https://docs.nestjs.com/websockets/gateways), [server-sent events](https://docs.nestjs.com/http/server-sent-events), or another mechanism.
...
However, there are edge cases where a request-based lifetime for a handler is desirable, such as per-request caching in GraphQL applications, request tracking, or multi-tenancy. See the [injection scopes](https://docs.nestjs.com/fundamentals/injection-scopes) chapter to learn how to control scopes.
