...
```typescript title="config.service.ts"
...
```typescript title="config.service.spec.ts"
import { TestBed } from '@suites/unit';
...
> info **Hint** The `mock()` function is an alternative to `createMock` from `@golevelup/ts-jest`. Both create typed mock objects. See the [testing fundamentals](https://docs.nestjs.com/fundamentals/testing#auto-mocking) chapter for more on `createMock`.
