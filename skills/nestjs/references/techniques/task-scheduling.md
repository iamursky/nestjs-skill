...
The `.forRoot()` call initializes the scheduler and registers any declarative <a href="https://docs.nestjs.com/techniques/task-scheduling#declarative-cron-jobs">cron jobs</a>, <a href="https://docs.nestjs.com/techniques/task-scheduling#declarative-timeouts">timeouts</a> and <a href="https://docs.nestjs.com/techniques/task-scheduling#declarative-intervals">intervals</a> that exist within your app. Registration occurs when the `onApplicationBootstrap` lifecycle hook occurs, ensuring that all modules have loaded and declared any scheduled jobs.
...
If you want to control your declarative interval from outside the declaring class via the <a href="https://docs.nestjs.com/techniques/task-scheduling#dynamic-schedule-module-api">Dynamic API</a>, associate the interval with a name using the following construction:
...
The <a href="https://docs.nestjs.com/techniques/task-scheduling#dynamic-intervals">Dynamic API</a> also enables **creating** dynamic intervals, where the interval's properties are defined at runtime, and **listing and deleting** them.
...
If you want to control your declarative timeout from outside the declaring class via the <a href="https://docs.nestjs.com/techniques/task-scheduling#dynamic-schedule-module-api">Dynamic API</a>, associate the timeout with a name using the following construction:
...
The <a href="https://docs.nestjs.com/techniques/task-scheduling#dynamic-timeouts">Dynamic API</a> also enables **creating** dynamic timeouts, where the timeout's properties are defined at runtime, and **listing and deleting** them.

## Dynamic schedule module API

The `@nestjs/schedule` module provides a dynamic API that allows you to manage declarative <a href="https://docs.nestjs.com/techniques/task-scheduling#declarative-cron-jobs">cron jobs</a>, <a href="https://docs.nestjs.com/techniques/task-scheduling#declarative-timeouts">timeouts</a> and <a href="https://docs.nestjs.com/techniques/task-scheduling#declarative-intervals">intervals</a>. The API also allows you to create and manage **dynamic** cron jobs, timeouts and intervals, where the properties are defined at runtime.
...
In this code, we use the `CronJob` object from the `cron` package to create the cron job. The `CronJob` constructor takes a cron pattern (just like the `@Cron()` <a href="https://docs.nestjs.com/techniques/task-scheduling#declarative-cron-jobs">decorator</a>) as its first argument, and a callback to be executed when the cron timer fires as its second argument. The `SchedulerRegistry#addCronJob` method takes two arguments: a name for the `CronJob`, and the `CronJob` object itself.
