...
If you would rather keep log content in your own aggregator, you do not have to forward anything: with `forwardLogs` off, the SDK still augments `ConsoleLogger` so every line carries its trace id, which is enough to jump from a line in your existing stack to the full trace in the dashboard. See the [SDK reference](https://docs.nestjs.com/observability/sdk) for both options and their redaction settings.
