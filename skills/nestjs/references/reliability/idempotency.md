> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/reliability/idempotency.md
...
- An `AuthGuard` that validates the bearer token and sets `req.user` (see [Authentication](https://docs.nestjs.com/security/authentication#implementing-the-authentication-guard)), plus a `@CurrentUser()` parameter decorator that reads it.
- A global `ValidationPipe`, and a TCP client for the shipping service, registered as `SHIPPING_SERVICE`.
- Its data in PostgreSQL, through Drizzle, with the database registered by `@nestjs/drizzle` (see [Drizzle](https://docs.nestjs.com/data/drizzle)). Using TypeORM? [Store keys in PostgreSQL](https://docs.nestjs.com/reliability/idempotency#store-keys-in-postgresql) ends with the store for TypeORM.
...
Errors can be stored too. [Tune TTLs and failure handling](https://docs.nestjs.com/reliability/idempotency#tune-ttls-and-failure-handling) covers which ones.
...
Until you [store keys in PostgreSQL](https://docs.nestjs.com/reliability/idempotency#store-keys-in-postgresql), records live in memory, in the default `InMemoryIdempotencyStore`. That's fine for a single instance in development, but every instance of the API has its own records, and they're lost on restart. When `NODE_ENV` is `production`, the app refuses to start this way, with an error that says how to register a store.
...
```typescript title="mobile/pay-order.ts"
...
```sql title="drizzle/0000_idempotency.sql.ts"
...
```typescript title="idempotency/drizzle-idempotency.store.ts"
...
The store is an ordinary provider. Register the database with `DrizzleModule` (see [Drizzle](https://docs.nestjs.com/data/drizzle)), and add the store to the providers of `AppModule`:
...
```typescript title="test/step-3.drizzle-store.spec.ts"
...
```typescript title="typeorm/idempotency-key.entity.ts"
...
```typescript title="typeorm/typeorm-idempotency.store.ts"
import { Injectable } from '@nestjs/common';
import {
  IdempotencyStorage,
  type IdempotencyAcquireResult,
...
```typescript title="test/typeorm-idempotency.store.spec.ts"
import { IdempotencyStorage } from '@nestjs/idempotency';
import { idempotencyStoreContract } from '@nestjs/idempotency/testing';
...
> info **Hint** Using Prisma? [The store contract](https://docs.nestjs.com/reliability/idempotency#the-store-contract), after the tutorial, shows how Prisma expresses these statements. Check your store with the same suite.
...
```typescript title="idempotency/redis-idempotency.store.ts"
...
```typescript title="test/step-4.redis-store.spec.ts"
import { idempotencyStoreContract } from '@nestjs/idempotency/testing';
// ...
...
3. When `ttl` has passed (48 hours, see [Tune TTLs and failure handling](https://docs.nestjs.com/reliability/idempotency#tune-ttls-and-failure-handling)), every record sealed with the old key has expired. Deploy `IDEMPOTENCY_KEYS=new-key`.
...
```typescript title="orders/orders.resolver.ts"
...
Deduplicating events needs an id that's the same for every delivery of an event, so it's derived from the data rather than generated on each send. An order is paid once, so `OrdersService` uses `order-paid:` followed by the order id. Mark the [event handler](https://docs.nestjs.com/microservices/basics#event-based) with `@Idempotent()`, and point `keyFrom` at that id:

```typescript title="shipping/shipping.controller.ts"
...
```typescript title="shipping/shipping.module.ts"
import { Module } from '@nestjs/common';
import { DrizzleModule } from '@nestjs/drizzle';
import { IdempotencyModule } from '@nestjs/idempotency';
import { drizzle } from 'drizzle-orm/node-postgres';
...
```typescript title="shipping/main.ts"
...
- Write the payment and the event in one transaction, with a [transactional outbox](https://docs.nestjs.com/reliability/outbox#prerequisites).
...
```typescript title="test/pay-order.e2e-spec.ts"
...
| --- | --- | --- | --- |
| `required` | `boolean` | `false` | Rejects calls without a key with `IDEMPOTENCY_KEY_REQUIRED`. See [Make the payment endpoint idempotent](https://docs.nestjs.com/reliability/idempotency#make-the-payment-endpoint-idempotent). |
...
| `ttl` | `Duration` | `'24h'` | How long a completed result is replayed. See [Tune TTLs and failure handling](https://docs.nestjs.com/reliability/idempotency#tune-ttls-and-failure-handling). |
...
| `encryption` | an object with `keys` | off | AES-256-GCM encryption of stored records. `keys` lists 32-byte `Buffer`s or strings of at least 32 characters, newest first. See [Encrypt stored receipts](https://docs.nestjs.com/reliability/idempotency#encrypt-stored-receipts). |
