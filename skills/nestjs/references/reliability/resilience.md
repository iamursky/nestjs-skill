> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/reliability/resilience.md
...
[Retry a POST safely](https://docs.nestjs.com/reliability/resilience#retry-a-post-safely) also uses `@nestjs/idempotency` (see [Idempotency keys](https://docs.nestjs.com/reliability/idempotency)).
...
```typescript title="shipping/carrier.client.ts"
...
```typescript title="shipping/shipping.service.ts"
import { Injectable } from '@nestjs/common';
...
```typescript title="shipping/repricing.service.ts"
...
Add `RepricingService` to the providers of `ShippingModule`, and call `repricePendingOrders()` from a scheduled job (see [Task scheduling](https://docs.nestjs.com/application/task-scheduling#declarative-cron-jobs)).
...
```typescript title="shipping/shipping.handlers.ts"
...
A timeout has `"code": "TIMEOUT"` and `"statusCode": 504`, and no `retryAfter`. In a hybrid application, connect the microservice with `inheritAppConfig: true`, or the global interceptor won't reach its handlers (see [Hybrid application](https://docs.nestjs.com/faq/hybrid-application#sharing-configuration)):
...
```typescript title="shipping/shipping.resolver.ts"
...
```typescript title="resilience.logger.ts"
...
[NestJS Observe](https://docs.nestjs.com/observability/overview#what-it-offers) already reads outbound HTTP from Node's diagnostics channels (see [Database queries and outbound HTTP](https://docs.nestjs.com/observability/sdk#database-queries-and-outbound-http)). It doesn't subscribe to the `nestjs:resilience` channels yet. Once it does, it can attach retries and timeouts to the request's span, chart breaker state per name, and alert on bulkhead rejections, with no code in either package that depends on the other.
...
```typescript title="test/shipping.e2e-spec.ts"
...
See [End-to-end testing](https://docs.nestjs.com/fundamentals/testing#end-to-end-testing) for the testing basics.
...
- **Register an idempotency store.** Without one, `@Idempotent()` keeps its records in each process's memory, lost on every restart or deploy, and production refuses to start. See [Store keys in PostgreSQL](https://docs.nestjs.com/reliability/idempotency#store-keys-in-postgresql) in the idempotency tutorial.
...
| --- | --- | --- | --- |
| `defaults` | `ResilienceDefaults` | none | Field defaults for `retry`, `timeout`, `circuitBreaker` and `bulkhead`. They fill in stages that a decorator or preset turns on, and never turn a stage on. See [Register the module](https://docs.nestjs.com/reliability/resilience#register-the-module). |
...
| --- | --- |
| `@Retry(options)` | `attempts` (3, including the first; `Infinity` retries until the signal aborts), `backoff`, `retryIf(error, attempt)` (every error except client errors), `idempotent` (see [Retry a POST safely](https://docs.nestjs.com/reliability/resilience#retry-a-post-safely)). `@Retry(5)` sets `attempts`, and `@Retry(false)` turns off an inherited retry. |
...
| `@CircuitBreaker(options)` | `name`, `failureRateThreshold` (50, a percentage), `minimumCalls` (10), `slidingWindow` (the last 20 calls: `type` is `'count'` or `'time'`, with a `size`), `openDuration` (`'30s'`), `halfOpenMaxCalls` (1), `recordIf(error)` (every error except client errors). A string is a breaker name. See [Fail fast with a circuit breaker](https://docs.nestjs.com/reliability/resilience#fail-fast-with-a-circuit-breaker). |
| `@Bulkhead(options)` | `name`, `maxConcurrent` (10), `maxQueue` (0), `queueTimeout` (no limit). A string is a bulkhead name. See [Cap concurrent CSV exports with a bulkhead](https://docs.nestjs.com/reliability/resilience#cap-concurrent-csv-exports-with-a-bulkhead). |
...
`ResilienceService` provides `preset(name)`, `create(options, name)`, `circuitBreaker(name)`, `bulkhead(name)`, `circuitBreakers()` and `bulkheads()`. See [Use the same policies inside services](https://docs.nestjs.com/reliability/resilience#use-the-same-policies-inside-services).

### Events

Every event is published on `ResilienceEvents.events$` and on the diagnostics channel `nestjs:resilience:<type>`. Each payload has `type`, `policy` and, when there is one, `source` (see [Observe breaker transitions](https://docs.nestjs.com/reliability/resilience#observe-breaker-transitions)).
...
On microservice and WebSocket handlers, the error is an `RpcException` or `WsException` whose payload has `status`, `code`, `statusCode`, `message` and, for rejections that know it, `retryAfter` in seconds. On GraphQL, the field's error has `code`, `httpStatus` and `retryAfter` in `extensions` (see [Other transports](https://docs.nestjs.com/reliability/resilience#other-transports)).
