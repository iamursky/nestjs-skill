> Source: https://github.com/nestjs/docs.nestjs.com/blob/master/content/reliability/outbox.md
...
- PostgreSQL, with [Drizzle ORM](https://orm.drizzle.team) on the `pg` driver, registered through [`@nestjs/drizzle`](https://docs.nestjs.com/data/drizzle). The outbox's messages commit with your rows, so they live in the database your orders live in, on a database server that outlives any instance of the application. Using TypeORM or Prisma? Follow the tutorial for what the application does, and take the store from [The store with TypeORM](https://docs.nestjs.com/reliability/outbox#the-store-with-typeorm) or [The store with Prisma](https://docs.nestjs.com/reliability/outbox#the-store-with-prisma).
- `@nestjs/microservices`, for the TCP link to the analytics service in [Publish to the analytics microservice](https://docs.nestjs.com/reliability/outbox#publish-to-the-analytics-microservice).
...
[`DrizzleModule`](https://docs.nestjs.com/data/drizzle) registers the database in the root module ([Register the outbox](https://docs.nestjs.com/reliability/outbox#register-the-outbox) shows the whole module). Its `forRootAsync()` factory runs once per application: the module calls the driver's `drizzle()` with the connection string and the schema, which opens a `pg` pool on `DATABASE_URL`, and ends that pool when the application shuts down:
...
> info **Hint** The tutorial reads `process.env` directly, to stay short. In an application, load the environment through [`@nestjs/config`](https://docs.nestjs.com/application/configuration) with a validation schema, so a missing `DATABASE_URL` stops the application at startup, and read it from `ConfigService` in the factory.
...
```typescript title="database/drizzle.ts"
...
```sql title="drizzle/0001_outbox.sql.ts"
...
```typescript title="database/drizzle-outbox.store.ts"
...
```typescript title="test/drizzle-outbox.store.e2e-spec.ts"
...
Register `OutboxModule` in the root module, after the `DrizzleModule` from [Set up the database](https://docs.nestjs.com/reliability/outbox#set-up-the-database), with the store in its `providers`. The feature modules it imports are built in the next steps:
...
- `transports` and `route`: where each message is published. The `local` transport is built in and runs `@OnOutboxMessage()` handlers in this process ([Handle order.placed in-process](https://docs.nestjs.com/reliability/outbox#handle-orderplaced-in-process)). `ClientProxyTransport(ANALYTICS_SERVICE)` publishes through the TCP client that `ClientsModule` registers under that name, which is why that module is in `imports` ([Publish to the analytics microservice](https://docs.nestjs.com/reliability/outbox#publish-to-the-analytics-microservice)). `route` picks exactly one transport per message.
- `relay`: the background publisher. It polls every `pollInterval`, leases what it claims for `lease`, and gives up on a single publish after `publishTimeout`. Setting `OUTBOX_RELAY=off` turns it off in API-only instances ([Run several instances](https://docs.nestjs.com/reliability/outbox#run-several-instances)).
- `retry`: 10 attempts, with exponential backoff that starts at 1 second and is capped at 1 minute ([Retries and the dead-letter queue](https://docs.nestjs.com/reliability/outbox#retries-and-the-dead-letter-queue)).
...
```typescript title="orders/order.ts"
...
```typescript title="notifications/mailer.service.ts"
...
```typescript title="notifications/order-emails.handler.ts"
import { Injectable } from '@nestjs/common';
...
```typescript title="inventory/stock-reservation.handler.ts"
import { Injectable, Logger } from '@nestjs/common';
import { InjectDrizzle } from '@nestjs/drizzle';
...
```typescript title="analytics-service/database/schema.ts"
...
It registers `DrizzleModule` as the order API does in [Set up the database](https://docs.nestjs.com/reliability/outbox#set-up-the-database), on its own `DATABASE_URL` and schema, and its migrations have their own drizzle-kit config:
...
```typescript title="analytics-service/database/drizzle-inbox.store.ts"
import { Injectable } from '@nestjs/common';
import { InjectDrizzle } from '@nestjs/drizzle';
...
```typescript title="analytics-service/analytics.module.ts"
import { Module } from '@nestjs/common';
import { DrizzleModule } from '@nestjs/drizzle';
...
```typescript title="analytics-service/order-stats.service.ts"
import { Injectable, Logger } from '@nestjs/common';
import { InjectDrizzle } from '@nestjs/drizzle';
...
```typescript title="analytics-service/analytics.controller.ts"
...
```typescript title="analytics-service/main.ts"
...
```typescript title="outbox-admin/outbox-admin.controller.ts"
import {
  Controller,
  Delete,
  Get,
...
```typescript title="outbox-admin/admin.guard.ts"
...
> info **Hint** `AdminGuard` is a placeholder. Dead letters contain full payloads, customer data included, so protect these routes with your application's real [authorization](https://docs.nestjs.com/security/authorization).
...
```typescript title="outbox-admin/outbox-metrics.service.ts"
...
```typescript title="test/orders.e2e-spec.ts"
...
- PGlite has a single connection, so it runs transactions one at a time. Test races between instances against a PostgreSQL server, the same major version as production, as the example does for [several instances](https://docs.nestjs.com/reliability/outbox#run-several-instances).
...
Order within a key is the order in which the store numbered the messages, not their ids: an id comes from its instance's clock, and clocks differ. PostgreSQL runs transactions concurrently, so the store makes the transactions that add messages with the same key take turns, and the numbering follows commit order (see [Write the outbox store](https://docs.nestjs.com/reliability/outbox#write-the-outbox-store)).

## Production checklist

- Keep the outbox's tables in your application's database, on a database server, and register a store for both contracts ([The store contract](https://docs.nestjs.com/reliability/outbox#the-store-contract)). Never keep them on the instance itself, in memory or in a file database such as SQLite on its disk: a container's filesystem goes away when the task is replaced, and every message that hadn't been published goes with it. The production guard refuses to start without a store; set `allowInMemoryStorage` only for a process whose messages you can afford to lose.
...
- Prune inboxes. Nothing calls `OutboxInbox.prune()` for you. Run it from a [cron job](https://docs.nestjs.com/application/task-scheduling#declarative-cron-jobs), for example `inbox.prune('30d')`, with a window longer than any redelivery, requeues included.
...
```typescript title="typeorm/outbox.entities.ts"
import type { OutboxAttempt, OutboxDeadLetterReason } from '@nestjs/outbox';
...
```typescript title="typeorm/data-source.ts"
...
```typescript title="typeorm/migrations/1790154527507-Outbox.ts"
...
```typescript title="typeorm/typeorm-outbox.store.ts"
import { Injectable } from '@nestjs/common';
import {
  OutboxStorage,
  OutboxTransactionRequiredError,
...
```typescript title="typeorm/app.module.ts"
import { Module } from '@nestjs/common';
import { OutboxModule } from '@nestjs/outbox';
...
```typescript title="typeorm/orders.service.ts"
@Injectable()
export class OrdersService {
  constructor(
    private readonly dataSource: DataSource,
...
```typescript title="test/typeorm-outbox.store.e2e-spec.ts"
/**
...
```prisma title="prisma/schema.prisma.ts"
...
```sql title="prisma/migrations/20260923091040_outbox/migration.sql.ts"
-- CreateTable
CREATE TABLE "outbox_messages" (
...
```typescript title="prisma/prisma-outbox.store.ts"
import { Injectable } from '@nestjs/common';
import {
  OutboxStorage,
  OutboxTransactionRequiredError,
...
```typescript title="prisma/database.module.ts"
...
```typescript title="prisma/app.module.ts"
import { Module } from '@nestjs/common';
import { OutboxModule } from '@nestjs/outbox';
import { NotificationsModule } from '../notifications/notifications.module.js';
...
```typescript title="prisma/orders.service.ts"
@Injectable()
export class OrdersService {
  constructor(
    private readonly prismaService: PrismaService,
...
```typescript title="test/prisma-outbox.store.e2e-spec.ts"
...
```typescript title="test/orm-orders.e2e-spec.ts"
...
`OutboxModule.forRoot()` takes these options. `forRootAsync()` takes `transports` classes, `imports` and `isGlobal` at its top level, and the rest from `useFactory`, `useClass` or `useExisting` (a class implementing `OutboxOptionsFactory`), as in [Register the outbox](https://docs.nestjs.com/reliability/outbox#register-the-outbox). The options token is `OUTBOX_MODULE_OPTIONS`. The store isn't an option: register it with `OutboxStorage`, as in [Write the outbox store](https://docs.nestjs.com/reliability/outbox#write-the-outbox-store). Durations are milliseconds, or strings such as `'30s'` and `'1m'`.
...
A message passed to `outbox.add(tx, message)` ([Save the order and its messages in one transaction](https://docs.nestjs.com/reliability/outbox#save-the-order-and-its-messages-in-one-transaction)) has these fields:
...
- `@OnOutboxMessage(topic, options)` takes `consumer` (required: the name of the handler's inbox) and `inbox` (default `true`; `false` runs every delivery). [Handle order.placed in-process](https://docs.nestjs.com/reliability/outbox#handle-orderplaced-in-process)
- A handler's context has `message`, `consumer`, `attempt`, `signal` and `processInTransaction(tx, work)`.
- `ClientProxyTransport(token, options)` emits the topic with an envelope of `id`, `topic`, `key`, `headers`, `createdAt` and `payload` (`OutboxEnvelope`). Its `toPacket(message, envelope)` option returns your own `pattern` and `data`. [Publish to the analytics microservice](https://docs.nestjs.com/reliability/outbox#publish-to-the-analytics-microservice)
...
| `OutboxDeadLetters` | `list(query)` (by `topic`, `key`, `limit`, `offset`; newest first), `get(id)`, `requeue(target)`, `purge(target)`. A target is an id, an array of ids, or a filter by `ids`, `topic`, `key`, `failedBefore`, or `all: true`. [Retries and the dead-letter queue](https://docs.nestjs.com/reliability/outbox#retries-and-the-dead-letter-queue) |
...
`OutboxEvents.events$` emits each event, and publishes it on its `node:diagnostics_channel` channel. Every payload has `type` and the `message`. [Run several instances](https://docs.nestjs.com/reliability/outbox#run-several-instances) counts them.
